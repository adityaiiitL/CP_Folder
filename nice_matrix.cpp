...
void solve()
{
    ...
    f(i,0,n/2){
        f(j,0,m/2){
            ...
            lli r=a[n-i-1][j];
            lli s=a[n-i-1][m-j-1];
            lli mn=abs(p-q)+abs(p-r)+abs(p-s);
            mn=min(mn,abs(q-p)+abs(q-r)+abs(q-s));
            mn=min(mn,abs(p-r)+abs(q-r)+abs(s-r));
    ...
    if(n&1){
        f(i,0,m/2){
            lli k=(a[n/2][i]+a[n/2][m-i-1])/2;
            ans+=abs(k-a[n/2][i])+abs(k-a[n/2][m-i-1]);
        }
    }
    if(m&1){
        f(i,0,n/2){
            lli k=(a[i][m/2]+a[n-i-1][m/2])/2;
            ans+=abs(k-a[i][m/2])+abs(k-a[n-i-1][m/2]);
        }
    }
    cout<<ans<<endl;
}
