...
bool comp(pair<pii,lli>p1,pair<pii,lli>p2){
    fo (p1.first).second<(p2.first).second;
}
void solve(){
    ...
    cin>>n>>m;
    vec v(n+1);
    vector<pair<pii,lli>>w;
    map<pair<pii,lli>,lli>mp;
    f(i,1,m+1){
        lli s,l,d;
        cin>>s>>l>>d;
        w.pb(mp(mp(s,l),d));
    ...
    f(i,0,m){
        lli c=0;
        lli s=(w[i].first).first,l=(w[i].first).second;
        lli d=w[i].second;
        f(j,s,l){
            if(c==d)break;
            if(v[j]==0)v[j]=mp[mp(mp(s,l),d)],c++;
        }
        if(c<d){
            cout<<-1<<endl;
            fo;
