...
void solve(){
    ...
    f(i,0,n){
        ...
        cin>>x>>y>>z;
        if(x==1)v.pb(mp(y,z));
        if(x==2)v.pb(mp(y,z-0.001));
        if(x==3)v.pb(mp(y+0.001,z));
        if(x==4)v.pb(mp(y+0.001,z-0.001));
    }
    lli c=0;
    f(i,0,n-1){
        f(j,i+1,n){
            ld x=v[i].first,y=v[i].second;
            ld a=v[j].first,b=v[j].second;
            if((x<=a and a<=y and y<=b) or (a<=x and x<=b and b<=y))c++;
            else if((x<=a and a<=b and b<=y) or (a<=x and x<=y and y<=b))c++;
        }
    }
    cout<<c<<endl;
}
