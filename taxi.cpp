...
void solve(){
    ...
    mi m;
    f(i,0,n)m[v[i]]++;
    lli c1=0,c2=0,c3=0,c4=0,c5=0;
    for(auto i:m){
        if(i.first==1)c1+=i.second;
        if(i.first==2)c2+=i.second;
        if(i.first==3)c3+=i.second;
        if(i.first==4)c4+=i.second;
        if(i.first>4)c5+=i.second;
    }
    lli c=c5+c4;
    if(c3<c1){
        c+=c3;
        ...
        if(c1%4==0){
            ...
            c+=c1/4+1;
            if((c2>0) and (c1%4==1 or c1%4==2))c2--;
            c+=c2/2;
            if(c2%2!=0)c++;
        }
    cout<<c<<endl;
