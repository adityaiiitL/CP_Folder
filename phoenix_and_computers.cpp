...
void build(){
    fact[0]=1;
    f(i,1,401)fact[i]=(fact[i-1]*i)%m;
}
lli func(lli i,lli cnt0,lli cnt1,bool f){
    if(i==n){
        fo (fact[cnt1]*fact[cnt0])%m;
    }
    if(f or i==n-1){
        fo func(i+1,cnt1+1,cnt0,0)%m;
    }
    else fo (func(i+1,cnt1+1,cnt0,0)+func(i+1,cnt1,cnt0+1,1))%m;
}
void solve()
{
    cin>>n>>m;
    build();
    cout<<func(0,0,0,1)%m<<endl;
}
