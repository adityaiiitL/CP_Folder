...
void solve(){
    ...
    in(a),in(t);
    if(k==1){
        f(i,1,n+1)cout<<t[0]+abs(a[0]-i)<<' ';
        br;
        fo;
    }
    vec v1,v2;
    mi m1,m2;
    f(i,0,k)m1[a[i]]=intmax;
    f(i,0,k){
        if(m1[a[i]]>t[i]){
            m1[a[i]]=t[i];
        }
    }
    for(auto i:m1)v1.pb(i.first);
    vec pre1,pre2;
    pre1.pb(m1[v1[0]]-v1[0]);
    pre2.pb(m1[v1[v1.size()-1]]+v1[v1.size()-1]);
    f(i,1,v1.size()){
        pre1.pb(min(m1[v1[i]]-v1[i],pre1[i-1]));
    }
    lli j=0;
    fr(i,v1.size()-2,0){
        pre2.pb(min(m1[v1[i]]+v1[i],pre2[j]));
        j++;
    }
    reverse(all(pre2));
    // debug(pre1);
    // debug(pre2);
    f(i,1,n+1){
        lli j=lower_bound(all(v1),i)-v1.begin();
        if(j==0){
            cout<<pre2[j]-i<<' ';
        }
        else if(j==v1.size()){
