...
vec v;
lli dp[2001][2001];
lli func(lli i,lli c,lli cnt){
    if(i==n and cnt<k)fo intmax;
    if(i==n)fo 0;
    if(dp[i][c]!=-1)fo dp[i][c];
    if(v[i]==(i+1-c)){
        fo dp[i][c]=min(1+func(i+1,c+1,cnt),func(i+1,c,cnt+1));
    }
    else fo dp[i][c]=min(func(i+1,c,cnt),1+func(i+1,c+1,cnt));
}
void solve(){
