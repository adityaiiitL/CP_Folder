...
void solve(){
    ...
    if(n&1){
        ans=(ans+(pow_itr((pow_itr(2,n-1)+1)%mod,k))%mod)%mod;
    }
    else {
        ans=(ans+pow_itr((pow_itr(2,n-1)-1)%mod,k))%mod;
        f(i,1,k+1){
            lli a=(pow_itr((pow_itr(2,n-1)-1)%mod,i-1))%mod;
            lli b=(pow_itr((pow_itr(2,n))%mod,k-i))%mod;
            ans=(ans+(a*b))%mod;
        }
    }
    cout<<ans%mod<<endl;
